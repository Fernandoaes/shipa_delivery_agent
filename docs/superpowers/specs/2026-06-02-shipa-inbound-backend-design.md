...
- **Order source is source-agnostic.** `sync.py` owns one upsert path: `orders` on `twin_order_ref`, `customers` deduped on phone. It is fed by whichever source we land on, all interchangeable:
  ...
  - **Decided (2026-06-03, pilot/demo):** the backend Postgres is the source of truth, seeded via `db/seed_twin_mock.sql` (SQL console, one-time). The HappyRobot platform **Twin** is a **non-secret read cache** populated by **polling `GET /twin/orders`** (webhook-auth) — *not* a direct SQL connection to Postgres (which would expose `otp_code`/PII at the boundary). Verification, OTP, status-at-decision, and all write actions stay on the live webhook tools; the Twin cache is reference/display only, so polling staleness is harmless.
  - `GET /twin/orders` returns a safe projection (excludes `otp_code` and `delivery_address`) plus an **operational overlay** (`reschedule_requested_date`, `escalated`, `investigation_open`) composed from the latest operation rows per order. This is what makes a mid-call reschedule/escalation visible to a polling Twin without mutating the order mirror or touching verified action code. Full snapshot, no cursor (small dataset).
  - For OTP and status the design *prefers* a **live read at call start** so the agent never acts on a stale code; the mock simulates this, and the read cache deliberately carries neither.
